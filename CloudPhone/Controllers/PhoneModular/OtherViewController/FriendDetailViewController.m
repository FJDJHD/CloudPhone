...
- (void)viewDidLoad {
    ...
    iconImageView.frame = CGRectMake(0, 0, MainWidth * 0.25,  MainWidth * 0.25);
    ...
    signayureLabel.textAlignment = NSTextAlignmentCenter;
    signayureLabel.textColor = [UIColor lightGrayColor];
    signayureLabel.text = @"个性签名，限定50字";
    signayureLabel.font = [UIFont systemFontOfSize:16.0];
    self.signayureLabel = signayureLabel;
    [self.view addSubview:signayureLabel];
    
    
    UIView *buttonBg = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.signayureLabel.frame) + 11, MainWidth, 44)];
   // buttonBg.backgroundColor = [UIColor redColor];
    ...
    dialDetailButton.frame = CGRectMake(0 , 0, MainWidth / 3.0, 40);
    dialDetailButton.center = CGPointMake(MainWidth / 2.0, CGRectGetMaxY(self.signayureLabel.frame) + 35);
    ...
    //发消息
    CGRect rect = CGRectMake(35, MainHeight - 20,( MainWidth - 35*2.0 - 10) / 2.0, 40);
    ...
    //发短信
    CGRect rect1 = CGRectMake(CGRectGetMaxX(sendInfoButton.frame) + 10, MainHeight - 20,( MainWidth - 35*2.0 - 10) / 2.0, 40);
    UIButton *sendInfoButton1 = [self getButtonWithString:@"发短信" rect:rect1 taget:self action:@selector(sendInfoButtonClick) ];
    [self.view addSubview:sendInfoButton1];
}

- (UITableView *)tableView {
    if (!_tableView) {
        CGRect tableViewFrame = CGRectMake(0, CGRectGetMaxY(self.dialDetailButton.frame)-38, MainWidth, MainHeight * 0.4);
